...
- `sudo`/root 권한이 있음
- 클라이언트에서 Data Relay Link 서버 public endpoint로 outbound 연결 가능
- 게시할 target 서비스가 실제로 실행 중이거나 클라이언트에서 접근 가능
- SSH를 게시한다면 SSH 계정과 `sshd`가 이미 준비됨

Data Relay Link는 OS user/password/SSH key/application service를 자동 생성하지 않습니다.

## Client lifecycle 한눈에 보기

기존 실사용 가이드의 lifecycle 그림은 Enrollment, 서비스 구성, 정상 운영, 업데이트와 제거의 관계를 빠르게 이해하는 데 유용합니다.

![Data Relay Link Client 관리 lifecycle](/images/guides/client-lifecycle.webp)
