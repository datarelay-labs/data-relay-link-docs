...
이 가이드는 빈 OCI 환경에서 시작해 **FRP Auto Deploy v2.2.1 서버를 만들고 첫 번째 SSH Client를 연결하는 단계까지** 한 번에 설명합니다. 화면은 기존 `frp.datarelay.run` OCI 구축 문서에서 실제로 사용했던 OCI Console 스크린샷을 재사용하고, FRP 명령과 동작 설명만 현재 stable 기준으로 업데이트했습니다.

<Note>
  OCI Console 메뉴명, Free Tier 정책, 리전별 capacity와 가격은 바뀔 수 있습니다. 실제 생성 직전 Console의 **Always Free Eligible** 표시와 예상 비용을 최종 기준으로 확인하세요.
</Note>
...
| 순서 | 작업 | 이 가이드의 값 |
| --: | --- | --- |
| 1 | VCN 생성 | `frp-vcn` / `10.0.0.0/16` |
| 2 | Internet Gateway + Route | `0.0.0.0/0` → Internet Gateway |
| 3 | Public Subnet | `frp-public-subnet` / `10.0.0.0/24` |
...
| 8 | FRP 설치 | immutable `v2.2.1` |
| 9 | 검증 | `frpctl doctor` + listener 확인 |
...
![그림 1. OCI Home 화면 - Networking/Instances 메뉴 진입](/images/oci/Picture1.png)
...
![그림 2. Virtual Cloud Networks에서 Create VCN 선택](/images/oci/Picture2.png)

VCN 이름은 `frp-vcn`, IPv4 CIDR은 `10.0.0.0/16`으로 설정합니다.

![그림 3. VCN 이름과 IPv4 CIDR 설정](/images/oci/Picture3.png)

### 2-2. Internet Gateway 생성

생성한 `frp-vcn`의 **Gateways** 탭에서 Internet Gateway를 생성합니다.

![그림 4. VCN의 Gateways 탭에서 Internet Gateway 생성](/images/oci/Picture4.png)

Internet Gateway 이름은 예제 기준 `frp-internet-gateway`입니다.

![그림 5. Internet Gateway 이름 예: frp-internet-gateway](/images/oci/Picture5.png)

### 2-3. Default Route Table에 인터넷 경로 추가

**Routing** 탭에서 `Default Route Table for frp-vcn`을 선택합니다.

![그림 6. VCN Routing 탭에서 Default Route Table 선택](/images/oci/Picture6.png)

**Route Rules → Add Route Rules**를 선택합니다.

![그림 7. Route Rules에서 Add Route Rules 선택](/images/oci/Picture7.png)
...
Target             frp-internet-gateway
```

![그림 8. Internet Gateway로 0.0.0.0/0 경로 추가](/images/oci/Picture8.png)

추가 후 Route Rule이 정상 반영되었는지 확인합니다.

![그림 9. Route Rule 적용 결과](/images/oci/Picture9.png)

### 2-4. Public Subnet 생성 시작

**Subnets** 탭에서 **Create Subnet**을 선택합니다.

![그림 10. VCN Subnets 탭에서 Create Subnet 선택](/images/oci/Picture10.png)
...
```text
Name           frp-public-subnet
Subnet Type    Regional
IPv4 CIDR      10.0.0.0/24
Route Table    Default Route Table for frp-vcn
Subnet Access  Public Subnet
```

![그림 11. Public Subnet 설정](/images/oci/Picture11.png)

### 2-6. Security List 설정

생성한 VCN의 **Security Lists** 메뉴로 이동합니다.

![그림 12. VCN Security Lists 메뉴](/images/oci/Picture12.png)

**Default Security List**의 Ingress Rules를 확인합니다.

![그림 13. Default Security List의 Ingress Rules](/images/oci/Picture13.png)

FRP Auto Deploy single-443 기준으로 TCP 443과 Published Service 범위인 TCP 6000-6098을 추가합니다. SSH 22는 기존 관리용 규칙을 유지하되 가능하면 관리자 공인 IP `/32`로 제한합니다.

![그림 14. Ingress Rule 추가 - TCP 443 및 TCP 6000-6098](/images/oci/Picture14.png)

추가 후 Ingress Rule이 정상 반영되었는지 확인합니다.

![그림 15. Ingress Rule 적용 결과](/images/oci/Picture15.png)
...
<Warning>
  **single-443에서는 6099와 7000을 OCI Public Ingress에 열지 않습니다.** 두 포트는 서버 내부 loopback backend입니다. 또한 다른 OCI 예제 화면의 `All Protocols` 규칙을 그대로 복사하지 말고 위 FRP용 최소 규칙을 사용하세요.
</Warning>
...
![그림 16. 생성한 네트워크 리소스 확인 후 Compute로 이동](/images/oci/Picture16.png)

### 3-1. Compute Instance 기본 정보

**Create compute instance** 화면에서 이름과 placement를 확인합니다.

![그림 17. Compute Instance 기본 정보](/images/oci/Picture17.png)
...
![그림 18. Ubuntu 이미지 선택](/images/oci/Picture18.png)

Security 설정은 별도 요구가 없다면 기본값을 사용합니다.

![그림 19. Security 설정 화면 - 기본값 사용](/images/oci/Picture19.png)

### 3-3. VCN과 Public Subnet 선택

Primary VNIC에서 앞에서 만든 `frp-vcn`과 `frp-public-subnet`을 각각 선택합니다.

![그림 20. 기존 frp-vcn과 frp-public-subnet 선택](/images/oci/Picture20.png)
...
![그림 21. Public IPv4 자동 할당 비활성화 및 SSH Key 생성/다운로드](/images/oci/Picture21.png)

### 3-5. Boot Volume 확인

별도 스토리지 요구사항이 없다면 Boot Volume은 기본값을 사용합니다.

![그림 22. Boot Volume 설정 - 기본값 사용](/images/oci/Picture22.png)

### 3-6. Instance 생성 전 Review

Image가 Ubuntu 24.04이고 Shape, VCN/Subnet, SSH Key가 의도한 값인지 확인한 뒤 Instance를 생성합니다.

![그림 23. Review - Ubuntu 24.04 및 VM.Standard.E2.1.Micro 확인](/images/oci/Picture23.png)

### 3-7. 생성된 Instance 및 Free/Eligible 표시 확인

Instance 목록에서 생성 상태를 확인합니다.

![그림 24. 생성된 Instance 목록](/images/oci/Picture24.png)
...
![그림 25. Instance의 Always Free 표시 확인 및 생성된 인스턴스 선택](/images/oci/Picture25.png)

중요한 부분은 다음입니다.

- 기존 `frp-vcn` 선택
- `frp-public-subnet` 선택
- Private IPv4는 자동 할당
...
![그림 26. Instance → Networking → Primary VNIC 확인](/images/oci/Picture26.png)

VNIC에서 **IP administration → Primary Private IP**를 선택합니다.

![그림 27. VNIC → IP administration → Primary Private IP](/images/oci/Picture27.png)

기존 Ephemeral Public IP가 있다면 Edit에서 **No public IP**를 선택해 먼저 해제합니다.

![그림 28. Edit Private IP Address에서 No public IP 선택](/images/oci/Picture28.png)

Public IP가 미할당 상태인지 확인합니다.

![그림 29. Public IP 미할당 상태](/images/oci/Picture29.png)
...
![그림 30. 새 Reserved Public IP 생성 및 연결](/images/oci/Picture30.png)

이 Reserved Public IP가 FRP Auto Deploy의 지속적인 공인 진입점이 됩니다. OCI에서는 Public IP가 Ubuntu NIC에 직접 설정되는 것이 아니라 OCI 측에서 private VNIC 주소와 매핑되기 때문에 Ubuntu의 `ip addr`에서는 `10.x.x.x` 같은 Private IP만 보이는 것이 정상입니다.

<Note>
  기존 Word 문서의 그림 31(Reserved Public IP 연결 완료)과 그림 32(SSH 접속 성공)는 실제 구축 당시의 공인 IP와 접속 정보가 화면에 포함되어 있어 공개 문서에서는 원본 이미지를 그대로 게시하지 않습니다. 아래 절차에서는 `<RESERVED-PUBLIC-IP>` placeholder를 사용합니다.
</Note>
...
# Part II — FRP Auto Deploy v2.2.1 설치
...
를 실행해 Public 443도 확인합니다. Client를 붙이기 전 `frpctl doctor`에 blocking failure가 없어야 합니다.
...
FRP Auto Deploy는 Client의 SSH 계정, password, SSH key, `sshd`를 생성하지 않습니다. `--ssh-user`의 계정과 SSH 서비스는 Client에 이미 존재해야 합니다.
...
- [ ] 외부 SSH 성공
- [ ] FRP Auto Deploy 2.2.1 / FRP 0.71.0 확인
- [ ] `frpctl doctor` blocking failure 없음
...
```bash
# 외부 workstation: Internet -> FRP Server
nc -vz -w 3 <RESERVED-PUBLIC-IP> <PUBLIC-SERVICE-PORT>

# FRP Client: Client -> Local/LAN Target
