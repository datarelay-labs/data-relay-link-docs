...
OCI Home에서 **Networking → Virtual Cloud Networks**로 이동합니다. 아래 화면은 기존 실구축 문서에서 사용했던 OCI Console 화면이며, 빨간 화살표와 실제 입력값을 그대로 참고할 수 있습니다.
...
![기존 실구축 OCI Console 화면: VCN, Internet Gateway, Route Table, Public Subnet, Security List](/images/guides/oci-networking.webp)

화면 순서는 기존 문서와 동일합니다.

1. **Virtual Cloud Networks → Create VCN**
2. `frp-vcn`, `10.0.0.0/16` 생성
3. **Gateways → Create Internet Gateway**
4. **Routing → Default Route Table → Add Route Rules**
5. `0.0.0.0/0`을 `frp-internet-gateway`로 전달
6. **Subnets → Create Subnet**
7. `frp-public-subnet`, `10.0.0.0/24`, VCN 기본 Route Table, Public Subnet 선택
