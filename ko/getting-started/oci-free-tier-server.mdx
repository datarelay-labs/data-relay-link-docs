...
다음 작업 단위에서 Word의 그림 11부터 이어서 Public Subnet 상세 설정과 Security List 화면을 배치합니다.
...
먼저 **Create compute instance** 화면에서 이름과 placement를 확인합니다.

![OCI Compute Instance 기본 정보](/images/oci/compute-basic.webp)

### 3-2. Canonical Ubuntu 24.04 선택

**Change image**를 눌러 Canonical Ubuntu를 선택하고 Ubuntu 24.04 LTS x86_64 이미지를 선택합니다. 이미지/버전은 OCI Console의 현재 표시를 최종 기준으로 확인하세요.

![OCI Canonical Ubuntu 24.04 이미지 선택](/images/oci/select-ubuntu.webp)

### 3-3. VCN과 Public Subnet 선택

Primary VNIC에서 앞에서 만든 `frp-vcn`과 `frp-public-subnet`을 각각 선택합니다.

![OCI frp-vcn 및 Public Subnet 선택](/images/oci/select-vcn-subnet.webp)
