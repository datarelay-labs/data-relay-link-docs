...
최종적으로 Reserved Public IP를 사용할 것이므로 **Automatically assign public IPv4 address**는 끕니다. 이어서 기존 SSH public key를 넣거나 OCI에서 새 key pair를 생성합니다.

![OCI Public IPv4 자동할당 해제 및 SSH Key 설정](/images/oci/public-ip-off-ssh-key.webp)

### 3-5. Instance 생성 전 최종 확인

마지막으로 Image, Shape, VCN/Subnet, SSH Key 등의 값을 검토한 뒤 Instance를 생성합니다.

![OCI Instance 생성 전 최종 확인](/images/oci/instance-review.webp)
...
Ephemeral public IP가 이미 붙어 있다면 먼저 **No public IP**로 해제합니다. 다시 Edit에서:
...
![기존 실구축 OCI Console 화면: VNIC IP administration, No public IP, Reserved Public IP 생성 및 연결](/images/guides/oci-reserved-ip.webp)
