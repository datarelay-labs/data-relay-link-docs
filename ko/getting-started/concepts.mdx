...
FRP, NAT, reverse tunnel을 잘 몰라도 이 페이지부터 보면 됩니다.
...
flowchart LR
    ...
    subgraph AFTER[FRP Auto Deploy]
      C[NAT 뒤 클라이언트] -->|아웃바운드 터널| S[FRP Auto Deploy 서버]
...
| **Client** | 보통 NAT/방화벽 뒤에서 `frpc`가 실행되는 원격 장비 | 지점 Linux 서버 |
...
2. **FRP control**은 reverse tunnel을 유지합니다.
