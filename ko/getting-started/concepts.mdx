---
title: "개념과 동작 원리"
description: "Data Relay Link의 서버, 클라이언트, 서비스, CLIENT ID, 공개 포트와 연결 흐름을 처음부터 쉽게 이해합니다."
...
Data Relay Link, NAT, reverse tunnel을 잘 몰라도 이 페이지부터 보면 됩니다.
...
flowchart LR
    ...
    subgraph AFTER[Data Relay Link]
      C[NAT 뒤 클라이언트] -->|아웃바운드 터널| S[Data Relay Link 서버]
...
| **Client** | 보통 NAT/방화벽 뒤에서 Data Relay Link Client runtime이 실행되는 원격 장비 | 지점 Linux 서버 |
...
flowchart TB
    ...
    C -->|2. Outbound Data Relay Link control| S
...
2. **Data Relay Link control**은 reverse tunnel을 유지합니다.
...
sequenceDiagram
    ...
    C->>S: Outbound Data Relay Link control 시작
```
