...
flowchart TD
    ...
    L -->|아니오| N[FRP tunnel / target protocol 상세 확인]
```
...
<Note>
  `doctor`는 읽기 전용입니다. Generated FRP config, registry, identity, PKI를 수동 수정하기 전에 먼저 실행하세요.
</Note>
...
FRP 없이 Client에서 target으로 먼저 직접 연결 가능해야 합니다.
...
FRP Auto Deploy는 hostname을 사용하지만 DNS record를 자동 생성하지 않습니다.
...
```text
public control       -> FRP control listen
public enrollment    -> allocator/enrollment listen
public service port  -> same FRP service port (1:1 권장)
```
