...
Enrollment는 Client와 Data Relay Link Server를 **처음 한 번 안전하게 연결하는 과정**입니다. 등록이 완료되면 persistent CLIENT ID와 management identity가 생성되며, 일반적인 reboot·서비스 변경·stable update 때문에 다시 등록할 필요는 없습니다.
...
- Data Relay Link는 OS user, password, SSH key, `sshd`, Windows RDP 설정을 자동 생성하지 않습니다.
