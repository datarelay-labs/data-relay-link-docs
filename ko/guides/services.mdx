...
| 사용 예 | FRP Client에서 보이는 Target | 설명 |
| --- | --- | --- |
...
LAN Target을 등록하기 전에 먼저 **FRP Client 자체에서** 해당 Target에 도달되는지 확인합니다.
...
여기서 실패하면 FRP 문제가 아니라 Client→LAN Target 사이의 routing, target firewall 또는 application 문제입니다. 이 경로를 먼저 해결한 뒤 Service를 게시하세요.

<Note>
  모든 LAN 서버에 FRP Client를 각각 설치할 필요는 없습니다. 하나의 Enrollment된 Client가 자신이 정상적으로 접근 가능한 내부 서버의 작은 gateway 역할을 할 수 있습니다. 다만 제품 목표 규모는 몇 대~몇십 대이므로 불필요하게 내부망 전체를 광범위하게 노출하지 마세요.
</Note>
