...
현재 published stable은 **FRP Auto Deploy v2.2.1**, pinned FRP는 **v0.71.0**입니다.
...
- update preservation
- reboot/autostart
- qualified release path의 backup/restore 및 Group MVP

## Server 범위

FRP Auto Deploy **Server는 Linux 기반**입니다. macOS와 Windows는 지원되는 Client 플랫폼이며 Windows FRP Auto Deploy Server는 현재 제품 범위가 아닙니다.
...
새 환경에서는 `sudo frpctl doctor`를 함께 사용하세요.
