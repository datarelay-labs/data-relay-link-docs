...
원격 **Client**에는 일반적으로 inbound port forwarding이 필요하지 않습니다. 반대로 **Data Relay Link Server 측 public entry point**는 control, enrollment, published service를 받을 수 있어야 합니다.
...
```mermaid
flowchart LR
    C[Remote Client\nNAT 뒤] -->|outbound 443 / 6099| S[Data Relay Link Server\nPublic IP]
...
flowchart LR
    ...
    F -->|6001 -> 6001| S[Private Data Relay Link Server]
```
...
| 항목 | Data Relay Link가 자동 설정? |
| --- | --- |
| Data Relay Link server/client config | **예** |
...
내부 사용자가 `fw.example.com`을 조회했을 때 public IP로 돌아가면 firewall이 hairpin NAT를 지원해야 내부 Data Relay Link server로 다시 들어올 수 있습니다.

```mermaid
flowchart LR
    I[Internal user] -->|public hostname| F[Firewall]
    F -. hairpin NAT needed .-> S[Internal Data Relay Link Server]
```
