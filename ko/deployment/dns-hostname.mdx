...
FRP Auto Deploy에서는 **Public IP**와 선택적 **Public Hostname**을 서로 다른 개념으로 봅니다.
...
Hostname metadata를 바꿔도 CLIENT ID, Service ID, public-port reservation, project CA, FRP control identity는 바뀌지 않습니다.
...
flowchart LR
    ...
    D -->|A / AAAA| P[Public FRP entry point]
    P --> S[FRP server 또는 firewall/NAT]
```

FRP Auto Deploy는 Route53, Cloudflare, DNSZi 같은 DNS provider API를 자동 호출하지 않습니다.
...
<Tip>
  IP + public port는 되는데 hostname만 실패한다면 FRP state보다 DNS, certificate, hairpin NAT를 먼저 확인하세요.
</Tip>
