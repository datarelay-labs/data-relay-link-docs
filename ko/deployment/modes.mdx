...
Data Relay Link의 server deployment mode는 **Direct**와 **Enterprise single-443** 두 가지입니다.

NAT는 별도 모드가 아니라 외부 network topology입니다.

![기존 실사용 가이드의 Data Relay Link 배포 방식 비교](/images/guides/deployment-modes.webp)
...
```mermaid
flowchart LR
    C[Remote Client] -->|Data Relay Link control TCP 443| S[Data Relay Link Server]
...
```mermaid
flowchart LR
    C[Remote Client] -->|HTTPS + Data Relay Link WSS TCP 443| FE[Public Frontend]
...
| Public Data Relay Link control | 443 | 443 via WSS |
| Public Enrollment HTTPS | 6099 | 443 |
| Published services | 6000-6098 | 6000-6098 |
| Internal 6099 | normal listen | loopback backend |
| Internal 7000 | normal path 아님 | loopback Data Relay Link backend |
