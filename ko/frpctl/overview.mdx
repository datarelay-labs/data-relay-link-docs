...
이 사이트의 운영 baseline은 **stable v2.1.2**입니다. `main`은 2.1.3 development이고 추가 lifecycle/retention command가 있을 수 있습니다.

Stable 서버에서는 현재 설치된 `frpctl help`와 immutable release 문서를 기준으로 하세요.

## 안전성

- `doctor`는 read-only
- `frpctl` history는 session-only
- `show upstream`은 정보용이며 최신 FRP를 자동 설치하지 않음
