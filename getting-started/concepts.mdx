...
description: "Understand Data Relay Link from first principles: server, client, service, identity, ports, and traffic flow."
...
If you are new to NAT, reverse tunnels, or relay gateways, start here. You do **not** need to understand relay-engine internals before using Data Relay Link.
...
flowchart LR
    ...
    subgraph AFTER[Data Relay Link]
      C[Client behind NAT] -->|outbound tunnel| S[Data Relay Link server]
...
| --- | --- | --- |
| **Server** | Public entry point that runs Data Relay Link server + management layer | `203.0.113.10` |
| **Client** | Remote machine running the Data Relay Link client runtime, usually behind NAT/firewall | branch Linux host |
...
Data Relay Link has three paths that beginners often mix together:
...
flowchart TB
    ...
    C -->|2. Outbound Data Relay Link control tunnel| S
...
2. **Data Relay Link control** keeps the reverse tunnel available.
...
## What Data Relay Link does not do
