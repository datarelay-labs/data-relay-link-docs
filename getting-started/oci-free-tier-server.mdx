...
![Picture 1. OCI Home - enter Networking / Instances](/images/oci/Picture1.png)
...
![Picture 2. Select Create VCN in Virtual Cloud Networks](/images/oci/Picture2.png)

Set the VCN name to `frp-vcn` and the IPv4 CIDR to `10.0.0.0/16`.

![Picture 3. Configure the VCN name and IPv4 CIDR](/images/oci/Picture3.png)

### 2-2. Create the Internet Gateway

In the new `frp-vcn`, open the **Gateways** tab and create an Internet Gateway.

![Picture 4. Create an Internet Gateway from the VCN Gateways tab](/images/oci/Picture4.png)

Use `frp-internet-gateway` as the example gateway name.

![Picture 5. Internet Gateway name example: frp-internet-gateway](/images/oci/Picture5.png)

### 2-3. Add the Internet route to the default route table

Open the **Routing** tab and select `Default Route Table for frp-vcn`.

![Picture 6. Select the default route table from the VCN Routing tab](/images/oci/Picture6.png)

Choose **Route Rules → Add Route Rules**.

![Picture 7. Add a route rule](/images/oci/Picture7.png)
...
![Picture 8. Add the 0.0.0.0/0 route through the Internet Gateway](/images/oci/Picture8.png)

Confirm that the route rule is present after saving.

![Picture 9. Confirm the applied route rule](/images/oci/Picture9.png)

### 2-4. Start creating the public subnet

Open the **Subnets** tab and select **Create Subnet**.

![Picture 10. Start creating the public subnet](/images/oci/Picture10.png)
...
![Picture 11. Configure the public subnet](/images/oci/Picture11.png)

### 2-6. Configure the Security List

Open the VCN's **Security Lists** page.

![Picture 12. Open the VCN Security Lists page](/images/oci/Picture12.png)

Review the ingress rules on the **Default Security List**.

![Picture 13. Review the Default Security List ingress rules](/images/oci/Picture13.png)

For FRP Auto Deploy in single-443 mode, add TCP 443 and the published-service range TCP 6000-6098. Keep SSH 22 for administration, preferably restricted to your administrator public IP `/32`.

![Picture 14. Add ingress rules for TCP 443 and TCP 6000-6098](/images/oci/Picture14.png)

Confirm that the ingress rules were applied.

![Picture 15. Confirm the applied ingress rules](/images/oci/Picture15.png)
...
![Picture 16. Confirm the network resources and open Compute](/images/oci/Picture16.png)
...
![Picture 17. Compute Instance basic settings](/images/oci/Picture17.png)
...
![Picture 18. Select the Ubuntu image](/images/oci/Picture18.png)

Unless your environment requires something different, keep the Security settings at their defaults.

![Picture 19. Security settings - use the defaults unless required otherwise](/images/oci/Picture19.png)

### 3-3. Select the VCN and public subnet

Under Primary VNIC, select the `frp-vcn` and `frp-public-subnet` created earlier.

![Picture 20. Select the existing frp-vcn and frp-public-subnet](/images/oci/Picture20.png)
...
![Picture 21. Disable automatic public IPv4 and create/download the SSH key](/images/oci/Picture21.png)

### 3-5. Review the boot volume

Unless you have a separate storage requirement, keep the boot-volume settings at their defaults.

![Picture 22. Boot Volume settings - keep the defaults unless required otherwise](/images/oci/Picture22.png)
...
![Picture 23. Review the Ubuntu 24.04 and VM.Standard.E2.1.Micro configuration](/images/oci/Picture23.png)

### 3-7. Confirm the created instance and Free/Eligible indicator

Confirm the instance state in the Compute Instances list.

![Picture 24. Confirm the created instance in the instance list](/images/oci/Picture24.png)
...
![Picture 25. Confirm the Always Free indicator and select the created instance](/images/oci/Picture25.png)
...
![Picture 26. Open the instance Networking page and Primary VNIC](/images/oci/Picture26.png)

In the VNIC, open **IP administration → Primary Private IP**.

![Picture 27. Open VNIC IP administration and the Primary Private IP](/images/oci/Picture27.png)

If an ephemeral public IP is currently attached, edit the private IP and select **No public IP** first.

![Picture 28. Select No public IP in Edit Private IP Address](/images/oci/Picture28.png)

Confirm that no public IP is currently assigned.

![Picture 29. Confirm that the public IP is unassigned](/images/oci/Picture29.png)
...
![Picture 30. Create and attach a new Reserved Public IP](/images/oci/Picture30.png)
...
<Note>
  Picture 31 (Reserved Public IP attached) and Picture 32 (successful SSH session) from the original Word guide contain the real public IP and SSH connection details from the field deployment. They are intentionally not published unchanged in this public documentation. The procedure below uses the `<RESERVED-PUBLIC-IP>` placeholder instead.
</Note>
