...
![FRP Auto Deploy client lifecycle](/images/guides/client-lifecycle.webp)

The current v2.2.1 rule is the same at a high level: enroll once, keep the persistent CLIENT ID, change services through `frpctl`, update without re-enrollment, and use the explicit lifecycle operations when retiring a client or releasing ports.
