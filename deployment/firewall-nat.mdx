...
The remote **client** normally does not need inbound port forwarding. The **FRP server side** still needs a reachable public entry point for control, enrollment, and published service ports.

![Direct, NAT, and Enterprise single-443 layouts from the original field guide](/images/guides/deployment-modes.webp)

The diagram is useful for separating **where the server lives** from **which FRP Auto Deploy mode it uses**. A private server behind DNAT can still run Direct mode; Enterprise single-443 is a different transport layout for constrained client networks.

## Topology A — server has a public IP

```mermaid
flowchart LR
    C[Remote client\nbehind NAT] -->|outbound 443 / 6099| S[FRP server\npublic IP]
...
In single-443, the public firewall only forwards the frontend control/enrollment endpoint on TCP 443. The internal backends remain loopback-only on the FRP server:

```text
Public firewall TCP 443  -> FRP server TCP 443 frontend
FRP server 127.0.0.1:7000 -> frps backend
FRP server 127.0.0.1:6099 -> enrollment/management backend
...
| Component | FRP Auto Deploy configures it? |
| --- | --- |
| FRP server/client config | **Yes** |
...
flowchart LR
    ...
    F -. hairpin required .-> S[Internal FRP server]
```
...
5. local `frpctl doctor` reports a healthy topology
