...
description: "Use a friendly DNS name for published services without changing Data Relay Link identity or control-plane semantics."
---
...
```mermaid
flowchart LR
    D[Your DNS provider] -->|A / AAAA record| P[Public Data Relay Link entry point]
    U[User] -->|fw.example.com| D
    P --> S[Data Relay Link server or firewall/NAT]
```
...
Data Relay Link does not issue or replace the published application's certificate. The **target application certificate** must be valid for the hostname users enter.
