...
| Use case | Target from the FRP client | Example |
| --- | --- | --- |
...
Before adding a LAN target, prove that the **FRP client itself** can reach it:
...
If this test fails, FRP cannot fix the client-to-LAN routing, target firewall, or target application. Resolve that path first, then publish the service.

<Note>
  You do not need a separate FRP client on every LAN host. A single enrolled client can act as a small gateway for hosts it can legitimately reach, which is useful for a few to a few dozen systems. Keep the scope deliberate rather than turning it into broad network exposure.
</Note>
