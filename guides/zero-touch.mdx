...
description: "Generate a one-time client bootstrap command from the Data Relay Link server."
---
...
sequenceDiagram
    ...
    C->>S: Start outbound Data Relay Link tunnel
```

The remote user does not need to understand Data Relay Link configuration. They run the exact generated command once.
...
It automates Data Relay Link onboarding, not the operating system's application/security configuration.
