---
title: "SSH"
description: "Publish SSH through Data Relay Link and connect using the assigned public service port."
---

# SSH

SSH is the most common Data Relay Link service.

## Local SSH on the client

```mermaid
flowchart LR
    U[Operator laptop] -->|ssh -p 6000| S[Data Relay Link Server]
    S -->|Data Relay Link tunnel| C[Remote Client]
...
The SSH account and `sshd` must already exist. Data Relay Link does not create users, passwords, keys, `authorized_keys`, or `sshd_config`.
...
```mermaid
flowchart LR
    U[Operator] -->|public port| S[Data Relay Link Server]
...
The Data Relay Link client must already have normal network reachability to `10.10.20.30:22`.
...
flowchart TD
    ...
    B -->|Yes| D{Can Data Relay Link Client reach target:22?}
