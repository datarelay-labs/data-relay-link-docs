...
description: "Publish arbitrary TCP services and services hosted on other LAN systems reachable from the Data Relay Link client."
---

# Custom TCP & LAN Targets

Data Relay Link is not limited to SSH/web. The stable core can publish arbitrary **TCP** targets reachable from an enrolled client.
...
```mermaid
flowchart LR
    U[Internet client] -->|assigned public port| S[Data Relay Link Server]
    S --> C[Data Relay Link Client]
    C -->|127.0.0.1:3000| G[Grafana]
```

## LAN gateway pattern

The target does not have to run on the Data Relay Link client itself.
...
The only Data Relay Link-specific requirement for the target path is that the Data Relay Link client can make a normal TCP connection to the configured `target-host:target-port`.

## Before publishing a LAN target

```mermaid
flowchart TD
    A[Want to publish LAN target] --> B{Can the Data Relay Link client reach\ntarget-host:target-port directly?}
    B -->|No| C[Fix routing / ACL / target firewall / listener]
    B -->|Yes| D[Add Data Relay Link service]
...
Publishing a database, appliance, or API makes that TCP endpoint reachable through the public Data Relay Link entry point. Data Relay Link does not automatically configure:
