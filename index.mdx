...
<CardGroup cols={2}>
  ...
  <Card title="I do not have a public IP or spare server" icon="cloud" href="/getting-started/oci-free-tier-server">
    Prepare an OCI Always Free-eligible Ubuntu VM with a persistent Reserved Public IPv4 address using step-by-step Console screenshots.
  </Card>
...
| Item | Current |
| --- | --- |
| Stable project release | **v2.2.1** |
| Project version | **2.2.1** |
| Pinned / tested FRP | **v0.71.0** |
| Release qualification | **Double Full Real E2E PASS / PASS** |
| Product repository `main` after PR #14 closure | `c7fe407d4dab12896469ef29cb7a882ad6311bd0` |
| Documentation/repository state | **CANONICAL / CLEAN** — Product Master restored via PR #12; README/CI housekeeping PR #14 merged; release/tag unchanged |
| Default deployment | **Direct** |

### Validated client platforms

| Platform | v2.2.1 validation |
| --- | --- |
...
| PowerShell 7 | CI validated; same-host Real E2E only when `pwsh` is installed |

The FRP Auto Deploy **server remains Linux-based**. Client validation and container portability are intentionally reported separately so a lower-level test is never presented as a stronger field claim.
