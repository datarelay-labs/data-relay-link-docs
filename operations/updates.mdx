...
`show upstream` is informational. FRP Auto Deploy does **not** automatically install the newest upstream FRP release. The current stable release remains pinned to the qualified version, **v0.71.0**.

```mermaid
flowchart LR
    U[Newest upstream FRP] -. informational .-> C[show upstream]
    P[Pinned/tested FRP 0.71.0] --> A[Allowed normal update path]
```
