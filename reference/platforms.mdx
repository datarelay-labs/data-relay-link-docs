...
Current published stable release: **FRP Auto Deploy v2.2.1** with pinned FRP **v0.71.0**.
...
- update preservation
- reboot/autostart
- backup/restore and Group MVP on the qualified release path

## Server scope

The FRP Auto Deploy **server is Linux-based**. macOS and Windows are supported client platforms; a Windows FRP Auto Deploy server remains outside the current product scope.
...
Platform support does not change the intended management scale: **approximately 1–50 clients**, especially a few systems to a few dozen. FRP Auto Deploy is not a hundreds/thousands-endpoint fleet-orchestration platform.
