...
Current published stable release: **Data Relay Link v2.2.1** with bundled/tested relay engine **v0.71.0**.
...
The Data Relay Link **server is Linux-based**. macOS and Windows are supported client platforms; a Windows Data Relay Link server remains outside the current product scope.
...
Platform support does not change the intended management scale: **approximately 1–50 clients**, especially a few systems to a few dozen. Data Relay Link is not a hundreds/thousands-endpoint fleet-orchestration platform.
